...
class User extends CI_model
{
	 ...
	 */
	public function get_all_free_users_info($page) {
		
		$myquery="
select  a.id as user_id,a.* ,p.*,s.title as skilltitle,st.title as startuptitle ,COALESCE(ed.grade, -1) as education,se.last_online ,h_ad.title as live ,h_ad.latlong as livelatlong,h_ad.security as livesecurity ,f_ad.title as born ,f_ad.latlong as bornlatlong,f_ad.security as bornsecurity , h_ad.category as livecity, f_ad.category as borncity,h_ad.iso as liveflag
from `stbl_user` as a 
...
left outer join
(select * from stbl_resume where type='p_education' order by grade desc)  ed on a.id=ed.user_id 
left outer join
...
";
	 ...
	 * @return int the user id
	 */
	public function get_all_users_info($ids,$page) {
...
";
	 ...
	 * @return int the user id
	 */
	public function get_user_info_by_key($userkey) {
...
group by a.id
	...
	public function get_all_free_users_info_sort_visit() {
		
		$myquery="select a.id as user_id, a.* ,p.*,s.title as skilltitle,st.title as startuptitle ,COALESCE(ed.grade, -1) as education ,se.last_online from `stbl_user` as a 
join stbl_user_profiles as p on a.id=p.user_id
left outer join
(select * from stbl_skills order by date desc)  s on a.id=s.user_id
left outer join
...
order by p.visitor desc
	...
	public function get_all_free_users_info_sort_rate() {
...
order by p.rate desc
	...
	public function get_all_free_users_info_sort_location() {
