...
class User_model extends CI_Model {
	...
	private function sendEmail($to_email)
        ...
        $from_email = 'hello@billionet.us'; //change this to yours
        $subject = 'Verify Your Email Address';
        $message = 'Dear User,<br /><br />Please click on the below activation link to verify your email address.<br /><br /><a href="'.base_url().'action/register/activator/' . $to_email['confirm_key'] . '">Click link</a><br /><br /><br />Thanks<br />Billionaire Team';
        ...
        //send mail
        $this->email->from($from_email, 'Billionaire');
        $this->email->to($to_email['email']);
        $this->email->subject($subject);
        $this->email->message($message);
        return $this->email->send();  			 
    }
    public function send_password_to_email($to_email,$link)
    	{
    	$this->load->library('email');
        $from_email = 'hello@billionet.us'; //change this to yours
        $subject = 'Billionaire--Reset password';
        $message = 'Dear User,<br /><br />Please click on the below reset password link to reset your account password.<br /><br /><a href="'.base_url().'signup/resetaccount/' . $link . '">Click link</a><br /><br /><br />Thanks<br />Billionaire Team';
...
/**
	...
	$id = $this->db->insert_id();
		$this->db->from('sessions');
		$this->db->join('user' , 'sessions.user_id = user.user_id');
...
SELECT *
FROM `btbl_user`
JOIN btbl_user_profiles ON `btbl_user`.`user_id` = btbl_user_profiles.user_id
WHERE btbl_user.email = '".$email."'" ;
	...
	public function get_user_full_info_by_username($username) {
		...
		$this->db->from('user');
		$this->db->join('user_profiles' , 'user.user_id = user_profiles.user_id');
	...
	public function check_update_user_active_link($link) {
		   ...
		   return $this->db->update('user', array('is_confirmed' => '1'));
		     
	}
		/**
	 * update_user_reset_password function.
	 * 
	 * @access public
	 * @param mixed $link
	 * @return int the user id
	 */
	public function update_user_reset_password($email,$link) {

			$this->db->where('email', $email);
		  	$this->db->update('user', array('new_password_requested' => '1','new_password_key'=> $link));
	 ...
	 * @return int the user id
	 */
	public function update_user_password($pass,$key) {

			$this->db->where('new_password_key', $key);
			$this->db->where('new_password_requested', 1);
		   return $this->db->update('user', array('new_password_requested' => '0','password'=> $this->hash_password($pass)));
	...
	public function get_user_quote_by_user_id($user_id) {
		...
		$this->db->from('quoted');
		$this->db->join('user', 'user.user_id = quoted.user_a');
...
/**
	...
	public function get_user_following_by_id($user_id) {
		...
		$myquery="select  c.*,a.* ,p.*,s.title as skilltitle,st.title as startuptitle ,COALESCE(ed.grade, 0) as education from `btbl_follow` as c 
join (select * from btbl_user ) a on a.id=c.user_a
join btbl_user_profiles as p on a.id=p.user_id
left outer join
(select * from btbl_skills order by date desc)  s on a.id=s.user_id
left outer join
(select * from btbl_startup order by startdate desc)  st on a.id=st.user_id 
left outer join
(select * from btbl_resume where type='education' order by startdate desc)  ed on a.id=ed.user_id 
...
";	
	...
	public function get_user_follower_by_id($user_id) {
		...
		$myquery="select  c.*,a.* ,p.*,s.title as skilltitle,st.title as startuptitle ,COALESCE(ed.grade, 0) as education from `btbl_follow` as c 
join (select * from btbl_user ) a on a.id=c.user_b
join btbl_user_profiles as p on a.id=p.user_id
left outer join
(select * from btbl_skills order by date desc)  s on a.id=s.user_id
left outer join
...
";	
	...
	public function get_user_follower_info($user_id) {
		...
		$this->db->from('follow');
		$this->db->join('user' , 'user.user_id = follow.user_a');
		$this->db->join('user_profiles' , 'user.user_id = user_profiles.user_id');
		$this->db->where('user_b', $user_id);
	...
	public function get_user_confirmd_by_id($user_id) {
		...
		$myquery="select  c.*,a.* ,p.*,s.title as skilltitle,st.title as startuptitle ,COALESCE(ed.grade, 0) as education from `btbl_confirm` as c 
join (select * from btbl_user ) a on a.id=c.user_a
join btbl_user_profiles as p on a.id=p.user_id
left outer join
(select * from btbl_skills order by date desc)  s on a.id=s.user_id
...
";	
	...
	public function get_user_secender_info($user_id) {
		$this->db->select("user_id,userkey,username,pic,firstname,lastname");
		$this->db->join('user' , 'user.user_id = confirm.user_b');
		$this->db->join('user_profiles' , 'user.user_id = user_profiles.user_id');
		$this->db->from('confirm');
...
/**
	...
	public function get_similar_user_skill_by_id($user_id) {
		...
		$query = $this ->db->get();
		if($query->num_rows() != 0 ){	
		$title="select user_id from btbl_skills where ";		
			foreach ($query->result_array() as $key => $value) {
				if($key == 0){
	...
	public function get_users_base_info_by_array($user_id) {
		...
		$this->db->from('user_profiles');
		$this->db->join('user' , 'user.user_id = user_profiles.user_id');
	...
	public function get_skills_locations($refrence_id) {
		...
		$myquery="
SELECT `title`,`latlong` FROM `btbl_locations`
 WHERE `type`='skill' and`refrence_id` = ".$refrence_id ;
	...
	public function get_resume_by_resume_id($id,$user_id) {
		$myquery="
		SELECT btbl_resume.* , re.title as location , re.latlong, re.category FROM `btbl_resume` 
left join (select * from btbl_locations where type='resume') as re on re.refrence_id=btbl_resume.`id`
	WHERE `btbl_resume`.`id` ='".$id."' and `btbl_resume`.`user_id` ='".$user_id."'"  ;
	...
	public function get_user_resumes_by_user_id($user_id) {
		...
		$myquery="
		SELECT btbl_resume.* , re.title as location , re.latlong FROM `btbl_resume` 
left join (select * from btbl_locations where type='resume') as re on re.refrence_id=btbl_resume.`id`
	WHERE `btbl_resume`.`user_id` ='".$user_id."'" ;
	...
	/**
	 * get_filtered_user_resumes_by_user_id function.
	 * 
	 * @access public
	 * @param mixed $user_id
	 * @return object the user object
	 */
	public function get_filtered_user_resumes_by_user_id($user_id,$category) {
		
		$myquery="
		SELECT btbl_resume.* , re.title as location , re.latlong FROM `btbl_resume` 
left join (select * from btbl_locations where type='resume') as re on re.refrence_id=btbl_resume.`id`
	WHERE `btbl_resume`.`user_id` ='".$user_id."' and `btbl_resume`.`type` ='".$category."'"  ;
...
SELECT * FROM `btbl_picture`
 WHERE `type`='resume' and`refrence_id` = ".$refrence_id ;
	...
	public function get_resume_skills($refrence_id) {
		...
		$myquery="
SELECT id , title as name FROM `btbl_resume_skills`
join btbl_skills on btbl_resume_skills.skill_id = btbl_skills.id
 WHERE `resume_id` = ".$refrence_id ;
	...
	public function get_skill_resumes($refrence_id) {
		...
		$myquery="
SELECT id , title as name FROM `btbl_resume_skills`
join btbl_resume on btbl_resume_skills.resume_id = btbl_resume.id
 WHERE `skill_id` = ".$refrence_id ;
	...
	public function get_startup_by_startup_id($id,$user_id) {
		$myquery="
		SELECT btbl_startup.* , re.title as location , re.latlong ,re.category FROM `btbl_startup` 
left join (select * from btbl_locations where type='startup') as re on re.refrence_id=btbl_startup.`id`
	WHERE `btbl_startup`.`id` ='".$id."' and `btbl_startup`.`user_id` ='".$user_id."'"  ;
	...
	public function get_user_startups_by_user_id($user_id) {
		...
		$myquery="
		SELECT btbl_startup.* , re.title as location , re.latlong FROM `btbl_startup` 
left join (select * from btbl_locations where type='startup') as re on re.refrence_id=btbl_startup.`id`
	WHERE `btbl_startup`.`user_id` ='".$user_id."'" ;
...
SELECT * FROM `btbl_picture`
 WHERE `type`='startup' and`refrence_id` = ".$refrence_id ;
	...
	public function get_startup_members($refrence_id) {
		...
		$myquery="
SELECT id  as name FROM `btbl_startup_member`
join btbl_user on btbl_startup_member.member_id = btbl_user.user_id
 WHERE `startup_id` = ".$refrence_id ;
	...
	public function get_member_startups($refrence_id) {
		...
		$myquery="
SELECT id , title as name FROM `btbl_startup_member`
join btbl_startup on btbl_startup_member.startup_id = btbl_startup.id
 WHERE `member_id` = ".$refrence_id ;
	...
	public function get_user_contacts_by_user_id($user_id,$type) {
		...
		$myquery="
		SELECT btbl_contact.* , re.title as location , re.latlong FROM `btbl_contact` 
left join (select * from btbl_locations where type='contact') as re on re.refrence_id=btbl_contact.`id`
	WHERE `btbl_contact`.`user_id` ='".$user_id."' and `btbl_contact`.`type` ='".$type."'";
